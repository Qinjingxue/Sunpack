...
def test_load_config_merges_simple_config_over_advanced_config(tmp_path, monkeypatch):
    ...
    payload["runtime"] = {"process_mode": "background"}
    ...
    assert config["runtime"]["process_mode"] == "high"
...
def test_load_config_rejects_invalid_runtime_process_mode(tmp_path, monkeypatch):
    ...
    advanced = tmp_path / "sunpack_advanced_config.json"
    payload = _advanced_payload()
    payload["runtime"] = {"process_mode": "turbo"}
    _write_json(advanced, payload)
    _write_json(simple, {})
    monkeypatch.setattr(loader, "_candidate_config_paths", _layered_config_paths(simple, advanced))

    with pytest.raises(loader.ConfigError, match="runtime.process_mode"):
        loader.load_config()
