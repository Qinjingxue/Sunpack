...
def _run_case(
    *,
    ...
    prior_environment = {
        ...
        for name in ("SUNPACK_SEVENZIP_PROFILE_READS",)
    }
    ...
    try:
        ...
        for run in range(runs):
            output = workspace.outputs / "prefetch" / f"run-{run}"
            job = _case_job(case, job_id=f"read-blocking-{run}", output_dir=output, dll_path=dll_path)
            try:
...
def _write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    columns = [
        ...
        "input_prefetch_consumer_wait_ms", "input_consumer_read_blocking_ms", "bytes_written", "files_written",
    ]
    ...
    with BenchmarkWorkspace(scenario, results_root=args.results_root, keep_workdir=args.keep_workdir) as workspace:
        ...
        report = {
            ...
            "method": {
                ...
                "prefetch_policy": "Profiles the production ZIP prefetch policy; the retired no-prefetch A/B baseline is no longer available.",
            },
