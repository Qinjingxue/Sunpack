...
下面是 **SunPack v0.7.0** 的测试结果，每个 case 测量 5 次、不预热。耗时和
进程树采样峰值 RSS 均为每个 case 的中位数；RSS 单位为 MiB。
...
18 个 case 的独立耗时中位数之和：SunPack 为 8,454.286 ms，`7z.exe` 为
10,656.967 ms（0.793x）。worker 在 18 个 case 的耗时中位数都更低。峰值
RSS 使用同一比例列比较；小于 1 表示 worker 占用更低，大于 1 表示 `7z.exe`
占用更低。BZip2/TBZ2 的 worker 峰值约 147 MiB，`7z.exe` 约 13 MiB。RSS
展示每个 case 的峰值中位数，不把互相独立的 case 内存相加。

---

## 注意事项

部分安装程序本身包含合法标准压缩包结构，从静态文件结构上可能无法与自解压归档可靠区分。SunPack 优先保证伪装归档与嵌入归档识别能力，因此某些自定义安装程序可能被识别为压缩包。项目不会以维护完整安装器特征库为目标。
