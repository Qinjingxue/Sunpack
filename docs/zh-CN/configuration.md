...
Native worker 以 CPU credit 作为解压并发预算。默认 nominal budget 等于逻辑核心数，每个已准入归档任务先占 1 个 base credit；格式不分配固定权重。SunPack worker context 下，handler 不再根据 CPU affinity/核心数决定 decoder 并发，LZMA2/XZ 也不再按 RAM 预算缩线程。decoder 在真正需要额外执行 lane 时向同一个 budget 申请 credits，同时保留有利于效率的算法级并行上限：RAR5 最多申请 8 个额外 worker credits，BZip2 会原子取得当前全部可用 extra credits，LZMA2/XZ 的 MtDec 则逐线程申请。7z 的多线程 mixer 继续按 7-Zip handler 的正常设置启用，其 pipeline 线程不单独计入 CPU credits。
