...
吞吐量控制器不再主动搜索最优并发。它只按实际写出吞吐被动检测外部环境变化：默认每 1 秒形成一个观察窗口；相对当前稳定基线下降至少 20% 时，effective CPU budget 按 `max(1, logical_processors / 8)` 降低一级；调整后的下一窗口建立新基线。之后吞吐若相对该基线提高至少 20%，按同样步长恢复，最高回到 nominal budget。稳定波动不会改变配额。
