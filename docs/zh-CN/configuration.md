...
Native worker 以 CPU credit 作为解压并发预算。默认 nominal budget 等于逻辑核心数，每个已准入归档任务先占 1 个 base credit；格式不分配固定权重。SunPack worker context 下，handler 不再根据 CPU affinity/核心数决定 decoder 并发，LZMA2/XZ 也不再按 RAM 预算缩线程。decoder 在真正需要额外执行 lane 时向同一个 budget 申请 credits，同时保留有利于效率的算法级并行上限：RAR5 不再使用输入大小门槛：只有一个已解析 block 时直接由 caller thread 处理，只有实际解析出多个彼此可并行的 block 形成 backlog 后才逐个申请额外 credit 并扩容，最多 8 个 worker；BZip2 的 worker 目标数直接由真实 queued + running ready-block 数决定，默认最多 12 个 worker，并保持每个最大 worker 对应 2 个 block 的固定解析前瞻窗口；单 block 不启动额外 worker；LZMA2/XZ 的 MtDec 同样逐线程申请。7z 的多线程 mixer 继续按 7-Zip handler 的正常设置启用。mixer pipeline 线程本身暂不单独计入 CPU credits，但会显式继承父任务的 CPU context，因此其中继续启动的 managed decoder 仍从同一个 job budget 申请 credits。CPU credit 释放改为边沿触发：只有共享 budget 从“已饱和”变为“出现可用 credit”时才最多唤醒 1 个准入 waiter，不再每次 release 都通知，也不会广播整个 worker pool。native executor 将前台/后台任务拆成两个队列，使“前台优先”准入变为 O(1)；decoder credit 记账也改为无分配 callback，仅保留每任务 current/peak 两项 telemetry。

不再使用吞吐量统计来寻找或修正并发。native worker 只在至少一个解压任务正在运行时读取系统物理内存：首个任务真正准入后立即采样一次，之后每 1 秒采样一次；worker 空闲时不轮询内存。低频内存/磁盘空间 controller 使用独立 mutex 和 condition variable，因此任务入队、准入、取消 bookkeeping 与普通完成路径不再为了监控争用 scheduler mutex。一个新的 active episode 只唤醒 controller 一次；同一 episode 内后续任务只更新 scheduler 状态，而磁盘空间进入 blocked 会直接唤醒 controller。若 `available_physical / total_physical < worker.minimum_available_memory_ratio`，effective CPU budget 每次按 `max(1, logical_processors / 8)` 降低一级，最低为 1；当可用内存回到阈值及以上时，按同样步长逐次恢复，最高回到 nominal budget。降档非抢占：已经持有的 credits 不会被撤销，只会阻止新的任务或 decoder lane 继续取得 credits。
