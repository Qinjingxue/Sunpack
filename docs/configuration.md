...
The native worker uses CPU credits as the extraction-concurrency budget. By default the nominal budget equals the logical processor count and every admitted archive job reserves one base credit; formats have no fixed CPU weights. In a SunPack worker context, archive handlers no longer derive decoder concurrency from CPU affinity/core count and LZMA2/XZ no longer reduce thread counts from RAM budgets. Decoders acquire execution lanes from the shared budget when work can actually run, while retaining algorithm-specific limits where they improve efficiency: RAR5 acquires up to 8 extra worker credits, BZip2 atomically takes all currently available extra credits, and LZMA2/XZ MtDec grows one credited thread at a time. The 7z multithreaded mixer remains enabled according to the normal 7-Zip handler setting; its pipeline threads are not separately accounted as CPU credits.
