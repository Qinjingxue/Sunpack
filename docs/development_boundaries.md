...
`sunpack.core.analysis` is the general archive analysis capability layer without business policy. Its public entry point `ArchiveAnalyzer` accepts a file, multi-volume, range, or segment source plus an `AnalysisRequest`, and outputs format evidence, fragment boundaries, confidence, and damage markers. `sunpack.core.analysis.embedded` owns the single full-carrier signature scan and exact embedded boundary resolution. After that scan, resolvers may read only bytes that can change the exact `[start,end)` result: 7z uses the Start Header, ZIP uses terminal geometry plus bounded link evidence, XZ uses footer/index geometry, and RAR/TAR/compression streams walk only syntax required to locate their terminator. Guessed boundaries from the next signature or EOF are forbidden; only exact ranges become extraction inputs. Header-encrypted RAR may decrypt header chains with the current password sources, and password-dependent boundary work is deliberately uncached. `probe_volume_anchor_paths` provides Relations with batched, bounded, read-only native volume evidence. Analysis must not depend on `ArchiveTask`, Detection, or the Coordinator, and must not write business knowledge.
