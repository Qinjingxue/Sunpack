...
| Command | Purpose |
| --- | --- |
| `extract`, `x` | Scan, identify, extract, verify, post-process, and clean up. |
| `watch`, `w` | Monitor directories and process files automatically once they are stable. |
| `scan`, `s` | Only scan and list extractable tasks; does not modify files. |
...
| `-m MODE`, `--process-mode MODE` | Temporarily override the shared RuntimeHost/native-worker process mode for this workload. Defaults to `high`. |
...
| `-P PASSWORD_FILE`, `--pw-file PASSWORD_FILE` | Read passwords from a text file, one per line. |
...
| `-f`, `--flatten` | Lift the contents of a single top-level directory after extraction. |
| `-F`, `--no-flatten` | Keep the extracted directory structure. |
| `--write-manifest`, `--manifest` | Write an extraction progress manifest into the output directory. |
| `-k`, `--allow-partial`, `--ap` | Accept partial recovery results as acceptable results. |
| `--direct-file`, `--direct` | Treat every input path directly as an archive, skipping directory scanning and automatic candidate discovery. |
...
| `status`, `st` | — | Show run status, pending counts, errors, and root directories. |
...
| `-P PASSWORD_FILE`, `--pw-file PASSWORD_FILE` | Read passwords from a text file, one per line. |
| `-a`, `--ask-pw` | Prompt for passwords interactively in the terminal. |
| `--no-builtin-pw`, `--no-bpw` | Do not use the built-in password table. |
| `--no-dir-pw`, `--no-dpw` | This command has no target archive and will not read a per-directory password file; in `extract` it disables per-directory passwords. |
