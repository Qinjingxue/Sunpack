        ...
        for directory, directory_paths in by_directory.items():
            ...
            for group in groups:
                if group.kind == "split_archive":
                    snapshot = self._snapshot(group, directory)
                    for owned_path in snapshot.owned_paths:
                        snapshots_by_path[path_key(owned_path)] = snapshot
                else:
                    ordinary_paths.update(path_key(path) for path in group.owned_paths)
            for path in directory_paths:
                key = path_key(path)
                selected = snapshots_by_path.get(key)
                if selected is not None or key in ordinary_paths:
